...
export function mineableHeaderStringBlake3(header: SerializedBlockTemplate['header']): Buffer {
...
export function mineableHeaderStringFishHash(
  header: SerializedBlockTemplate['header'],
): Buffer {
  const bw = bufio.write(MINEABLE_BLOCK_HEADER_SIZE)
  bw.writeBytes(Buffer.from(header.graffiti, 'hex'))
  bw.writeU32(header.sequence)
