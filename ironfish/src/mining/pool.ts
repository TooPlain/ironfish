...
export class MiningPool {
  ...
  async submitWork(
    ...
    // Best effort handling of v1 vs v2 here, use the graffiti if it was provided, otherwise the default.
    if (graffiti.length === 0) {
      // Use default
      graffiti = client.graffiti.toString('hex')
    } else if (graffiti.length !== 64) {
      const l = graffiti.length
      const msg = `Client ${client.id} work with invalid custom graffiti length: ${l}`
